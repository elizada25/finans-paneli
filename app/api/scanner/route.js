...
function findCross({ history, fastPeriod, slowPeriod, direction }) { const rows = history.rows; const closes = rows.map((row) => Number(row.close)); if (closes.length < Math.max(fastPeriod, slowPeriod) + 5) return null; const fastEma = calculateEma(closes, fastPeriod); const slowEma = calculateEma(closes, slowPeriod); const lastIndex = rows.length - 1; const previousIndex = lastIndex - 1; const fastNow = fastEma[lastIndex]; const slowNow = slowEma[lastIndex]; const fastPrev = fastEma[previousIndex]; const slowPrev = slowEma[previousIndex]; if (![fastNow, slowNow, fastPrev, slowPrev].every(Number.isFinite)) return null; const crossedUp = fastPrev <= slowPrev && fastNow > slowNow; const crossedDown = fastPrev >= slowPrev && fastNow < slowNow; if (direction === "up" && !crossedUp) return null; if (direction === "down" && !crossedDown) return null; const latestRow = rows[lastIndex]; const previousClose = Number(rows[previousIndex]?.close); const dailyChange = Number.isFinite(previousClose) && previousClose !== 0 ? ((latestRow.close - previousClose) / previousClose) * 100 : null; return { symbol: history.symbol, price: latestRow.close, dailyChange, fastEma: fastNow, slowEma: slowNow, differencePercent: ((fastNow - slowNow) / slowNow) * 100, volumeRatio: getVolumeRatio(rows), signalDate: latestRow.timestamp ? new Date(latestRow.timestamp * 1000).toISOString() : null, direction }; }
