...
function findCross({ history, fastPeriod, slowPeriod, direction }) { const rows = history.rows; const closes = rows.map((row) => row.close); if (closes.length < Math.max(fastPeriod, slowPeriod) + 5) return null; const fastEma = calculateEma(closes, fastPeriod); const slowEma = calculateEma(closes, slowPeriod); const lastIndex = rows.length - 1; const previousIndex = lastIndex - 1; const fastCurrent = fastEma[lastIndex]; const slowCurrent = slowEma[lastIndex]; const fastPrevious = fastEma[previousIndex]; const slowPrevious = slowEma[previousIndex]; if (!Number.isFinite(fastCurrent) || !Number.isFinite(slowCurrent) || !Number.isFinite(fastPrevious) || !Number.isFinite(slowPrevious)) return null; const crossedUp = fastPrevious <= slowPrevious && fastCurrent > slowCurrent; const crossedDown = fastPrevious >= slowPrevious && fastCurrent < slowCurrent; if (direction === "up" && !crossedUp) return null; if (direction === "down" && !crossedDown) return null; const latestRow = rows[lastIndex]; const previousClose = rows[previousIndex]?.close; const dailyChange = Number.isFinite(previousClose) && previousClose !== 0 ? ((latestRow.close - previousClose) / previousClose) * 100 : null; const volumeRatio = getVolumeRatio(rows); return { symbol: history.symbol, price: latestRow.close, dailyChange, fastEma: fastCurrent, slowEma: slowCurrent, differencePercent: ((fastCurrent - slowCurrent) / slowCurrent) * 100, volumeRatio, signalDate: latestRow.timestamp ? new Date(latestRow.timestamp * 1000).toISOString() : null, direction }; }
