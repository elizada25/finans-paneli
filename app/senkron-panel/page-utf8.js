...
<section style={styles.chartSection}>
  <div style={styles.sectionHeader}>
    ...
    <span style={styles.stockCount}>TradingView</span>
  </div>
...
function PortfolioSection({
  ...
  async function sellPosition(stock) {
    const confirmed = window.confirm(
      `${stock.code} pozisyonunu portfÃ¶yden kaldÄ±rmak istiyor musunuz?`
    );
    ...
    try {
      ...
      console.error('Pozisyon silme hatasÄ±:', error);
      window.alert(
        `Pozisyon kaldÄ±rÄ±lamadÄ±: ${
          error?.message || 'Bilinmeyen hata'
        }`
      );
    } finally {
